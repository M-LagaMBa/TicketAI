...
1. Abra um ticket no Hubspot e mude o **Status** para **Fechado**, até aparecer o modal **"Propriedades dependentes"**.
2. Clique no ícone do TicketAI para abrir o widget.
3. Clique no preset correspondente ao caso.
4. A extensão preenche **Descrição → Produto → Categoria → Assunto**, na ordem certa, respeitando os campos dependentes.
5. Precisou ajustar algo específico daquele ticket? É só editar direto no campo do Hubspot.
...
| `Ctrl` + `N` | Abre o formulário de novo preset |

> ⚠️ `Ctrl+N` pode ser reservado pelo navegador para "nova janela" e não chegar até a extensão. Se isso acontecer no seu teste, me avisa que trocamos por outra combinação.

---

## 🖱️ Funcionalidades

- 🗂️ **Presets completos** — Nome, Grupo, Descrição, Produto, Categoria e Assunto, tudo preenchido de uma vez.
...
- 📦 **Exportar / Importar** — backup dos presets em JSON.
- 🖥️ **Widget flutuante** — arrasta, redimensiona e minimiza, com posição salva entre recarregamentos.
- 📎 **Modo compacto** — mostra um preset por vez, navegação por scroll do mouse.
...
- `Ctrl+N` pode ser interceptado pelo navegador antes de chegar à extensão. Se confirmado em teste, trocar por outra combinação (ex: `Alt+N`).
